...
## 🍿 회고
### ReactorKit을 통한 단방향 데이터 플로우 구현
- 이전 프로젝트에서 활용하였던 TCA 아키텍처와 같이 MVI 패턴과 유사한 단방향 데이터 플로우 패턴으로 구현할 수 있었습니다.
- 그러나 TCA에서 일관된 구조를 제공해주었던 점과 달리 ReactorKit에서는 개발자가 코드 구조를 고려하며 개발을 진행해야하는 차이점을 느낄 수 있었습니다.
- 추가적으로 앱 기능에 대한 로직 구현에 방향을 잡는 것에 시간을 많이 할애하였고, 목표한 데드라인을 위해 빠른 속도로 개발하였기 때문에 코드 및 파일 구조가 정돈되지 않은 채 출시를 하게 되었습니다.
- 추후 지속적인 기능 개발 및 이슈 개선을 진행하면서 정돈되지 않은 구조를 클린 아키텍처나 다른 형태의 아키텍처 패턴을 더 고민하며 리팩토링도 함께 진행하려고 합니다.
