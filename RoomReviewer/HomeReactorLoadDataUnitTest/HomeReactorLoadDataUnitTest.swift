...
final class HomeReactorLoadDataUnitTest: XCTestCase {
    ...
    func test_SuccessLoadData() {
        ...
        reactor = HomeReactor(networkService: mockNetworkManager, mediaDBManager: mediaDBManager, networkMonitor: mockNetworkMonitor)
        ...
        XCTAssertEqual(finalState?.sections.count, 2)
        
        let tvItems = finalState?.sections.first?.items ?? []
